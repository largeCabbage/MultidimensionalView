package com.dabaicai.multidimensionview;
...
public class MultidimensionalView extends View {
     ...
     * 所有背景各顶点数组
     */
    private PointF[][] mAllDeepBgPoints;
     ...
     * 图层份数
     */
    public final static int  sLayerNum =5;
     ...
     * 画每个维度图层背景
     */
    private void drawEveryDeepBackground(Canvas canvas,int factor,@ColorInt int color) {
        initDeepBgPoints(factor);
        mBgPath.reset();
        PointF[] pointF = mAllDeepBgPoints[factor];

        mBgPath.moveTo(pointF[0].x, pointF[0].y);//第一个点 顶点
        for (int i = 1; i < mPointCounts; i++) {
            mBgPath.lineTo(pointF[i].x, pointF[i].y);
        }
        mBgPath.close();
    ...
    private void drawDeepBackground(Canvas canvas) {
        ...
        mBgPath.close();
        mBgPaint.setColor(getContext().getResources().getColor(R.color.bg_blue_deep_color1));
        mBgPaint.setAlpha(mPaintAlpha);
        canvas.drawPath(mBgPath, mBgPaint);
    }
     ...
     * 初始化每个维度深色背景各顶点
     */
    private void initDeepBgPoints(int factor) {
        initBgPoints();
        mDeepBgPoints = new PointF[mPointCounts];
        mAllDeepBgPoints = new PointF[mPointCounts][];
        for (int i = 0; i < mPointCounts; i++) {
            mDeepBgPoints[i] = getEveryLayerPointFromCentre(mBgPoints[i],factor);
        }

        mAllDeepBgPoints[factor] = mDeepBgPoints;
    }
     ...
     * @return
     */
    private PointF getEveryLayerPointFromCentre(PointF p1,float factor) {
        if (factor==0){
            factor=0.5f;
        }else {
            factor=factor/sLayerNum;
        }

        float x,y;
        if(p1.x>centerPoint.x){
            x = p1.x-(p1.x-centerPoint.x)*factor;
        }else {
            x = p1.x +(centerPoint.x - p1.x)*factor;
        }

        if(p1.y>centerPoint.y){
            y = p1.y-(p1.y-centerPoint.y)*factor;
        }else {
            y = p1.y + (centerPoint.y - p1.y)*factor;
        }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        drawEveryLayerBackground(canvas,1,getResources().getColor(R.color.bg_blue_deep_color1));
        drawEveryLayerBackground(canvas,2,getResources().getColor(R.color.bg_blue_deep_color2));
        drawEveryLayerBackground(canvas,3,getResources().getColor(R.color.bg_blue_deep_color3));
        drawEveryLayerBackground(canvas,4,getResources().getColor(R.color.bg_blue_deep_color4));
    ...
    private void drawEveryLayerBackground(Canvas canvas, @IntRange (from = 1,to = sLayerNum-1) int factor, @ColorInt int color) {

        //画深色背景
        drawEveryDeepBackground(canvas,factor,color);
        //画网格 中心到各顶点线
        drawMesh(canvas);
